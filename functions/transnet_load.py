...
def create_transmat_from_adjmat(adj_mat, vacc_dict, nodelist, R0=15., q=0.5):
    #print 'making fsvec'
    FSLIST = [models.op_fs(p, R0) for p in np.arange(0, 1., 0.001)]
    #fs_vec = np.array([ models.op_fs(1.-vacc_dict[n], R0) for n in nodelist])
    fs_vec = np.array([FSLIST[int((1.-vacc_dict[n])*1000)] for n in nodelist])
   # print 'making vacvec'
...
def create_network_from_transmat(trans_mat, nodelist):
    trans_net = nx.from_numpy_matrix(trans_mat, create_using=nx.MultiDiGraph())
    rename_dict = dict([[i, nodelist[i]] for i in range(len(nodelist))])
    trans_net = nx.relabel_nodes(trans_net, rename_dict)
    return trans_net

def create_network_from_transmat_sparse(trans_mat, nodelist):
    trans_net = nx.from_numpy_matrix(trans_mat, create_using=nx.MultiDiGraph())
    rename_dict = dict([[i, nodelist[i]] for i in range(len(nodelist))])
    trans_net = nx.relabel_nodes(trans_net, rename_dict)
    return trans_net
...
def plot_transnet(trans_net, pos, highlight=[] , title='transmission network of the Nethelands', sizes='deg'):
    fig = plt.figure(figsize=[14,20])
    ax = fig.add_subplot(111)
    highlight_net = trans_net.subgraph(nbunch=highlight)
    if sizes == 'deg':
    ...
    nx.draw_networkx_nodes(trans_net, weight='weight', pos=pos, node_size=np.array([sizes[n] for n in trans_net.nodes()])*5., node_color='cyan', alpha=0.7, linewidths=0.2, ax=ax)
    nx.draw_networkx_nodes(highlight_net, weight='weight', pos=pos, node_size=np.array([sizes[n] for n in highlight_net.nodes()])*5., node_color='Red', alpha=0.7, linewidths=0.2, ax=ax)
    nx.draw_networkx_edges(trans_net, weight='weight', pos=pos, edgelist=trans_net.edges(), width=np.array([edge_dict[e + (0,)] for e in trans_net.edges()]), edge_color='k', ax=ax, arrows=False)
    ax.set_aspect('equal')
...
def make_yso_set(adj_mat, vacc_dict, years_since_ob, school_data, nodelist, years_in_pri=7., years_in_sec=7., post_outbreak=0.98):
    ...
    nodes = np.intersect1d(nodelist, school_data.BRIN)
    trans_net_yso = trans_net_yso.subgraph(nbunch=nodes)
